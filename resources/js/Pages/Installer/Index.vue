            ...
            <div class="mt-20">
                ...
                <div v-if="props.step === null" class="md:max-w-[40em] p-2">
                    ...
                    <div class="bg-slate-100 rounded-md p-4">
                        <p class="mb-2">{{ $t('Step 1: You will need the following information before proceeding:') }}
                        </p>
                    ...
                    <div class="bg-slate-100 rounded-md p-4 mt-2">
                        <p class="mt-4 mb-2">{{ $t(`Step 2: You will need to provide the following information to setup
                        ...
                        <div class="mt-4 p-2 bg-slate-200 border-l-[2px] border-red-500">{{ $t(`If you need more help
                            you can consult the installation guide`) }}</div>
                    </div>
                    <div class="flex justify-center">
                        <Link href="/install/server"
                            class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm">{{ $t('Next Step')
                            }}</Link>
                    </div>
                </div>
                <div v-if="props.step === 'server'" class="w-[25em] md:w-[30em] p-2">
                    ...
                    <div class="bg-slate-100 rounded-md p-4">
                        ...
                        <div class="bg-slate-200 px-2 rounded-md text-sm">
                            <div v-for="(item, index) in props.system.requirements"
                                class="row border-b border-white py-2">
                                <div class="flex items-center w-3/4 gap-x-2">
                                    <span>
                                        <svg v-if="!item" class="text-red-700" xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <svg v-else class="text-green-700" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div class="flex justify-between gap-x-2">
                        ...
                        <Link href="/install/folders"
                            class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm">
                ...
                <div v-if="props.step === 'folders'" class="md:w-[30em] p-2">
                    ...
                    <div class="bg-slate-100 rounded-md p-4">
                        ...
                        <div class="bg-slate-200 px-2 rounded-md text-sm">
                            <div v-for="(item, index) in props.folders.permissions"
                                class="row border-b border-white py-2">
                                <div class="flex items-center w-3/4 gap-x-2">
                                    <span>
                                        <svg v-if="!item" class="text-red-700" xmlns="http://www.w3.org/2000/svg"
                ...
                <div v-else-if="props.step === 'database'" class="mt-5 px-4">
                    <form v-if="!isValidPurchaseCode" class="w-80" @submit.prevent="validateCode()">
                        ...
                        <div class="flex justify-between gap-x-2 mt-2">
                            ...
                            <button v-if="!isLoading" href="/install/database" type="submit"
                                class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm">{{ $t(`Next
                                Step`) }}</button>
                    ...
                    <form v-else @submit.prevent="checkDb()">
                        ...
                        <div v-if="props.flash?.status?.message"
                            class="mt-4 p-2 bg-slate-200 border-l-[2px] border-red-500 my-4 max-w-[28em] text-sm">{{
                                props.flash.status.message }}</div>
                        <div class="flex gap-x-4 mb-4">
                            <FormInput v-model="form.host" :name="$t('Host Name')" :error="form.errors.host"
                                :type="'text'" :placeholder="'localhost'" :class="'w-3/4'" />
                            <FormInput v-model="form.port" :name="$t('Port')" :error="form.errors.port" :type="'text'"
                                :placeholder="'3306'" :class="'w-1/4'" />
                        </div>
                        <FormInput v-model="form.dbname" :name="$t('DB Name')" :error="form.errors.dbname"
                            :type="'text'" :placeholder="'Database name'" :class="'sm:col-span-6 mb-4'" />
                        <FormInput v-model="form.dbprefix" :name="$t('DB Prefix')" :error="form.errors.dbprefix"
                            :type="'text'" :placeholder="'Database prefix'" :class="'sm:col-span-6 mb-4'" />
                        <FormInput v-model="form.dbuser" :name="$t('DB Username')" :error="form.errors.dbuser"
                            :type="'text'" :placeholder="'Database username'" :class="'sm:col-span-6 mb-4'" />
                        <FormInput v-model="form.dbpass" :name="$t('DB Password')" :error="form.errors.dbpass"
                            :type="'password'" :placeholder="'Database password'" :class="'sm:col-span-6 mb-4'" />
                ...
                <div v-else-if="props.step == 'app'" class="md:w-[30em] p-2">
                    ...
                    <form @submit.prevent="setCompanyDetails()">
                        <FormInput v-model="form2.company_name" :name="$t('App Name')" :type="'text'"
                            :error="form2.errors.company_name" :placeholder="'App name'"
                            :class="'sm:col-span-6 mb-4'" />
                        <FormInput v-model="form2.url" :name="$t('Project Url')" :type="'url'" :error="form2.errors.url"
                            :placeholder="'https://mytestapp.com'" :class="'sm:col-span-6 mb-8'" />
                        <hr class="my-4" />
                        <div class="flex gap-x-4 mb-4">
                            <FormInput v-model="form2.first_name" :name="$t('Admin First Name')"
                                :error="form2.errors.first_name" :type="'text'" :placeholder="'First Name'"
                                :class="'w-1/2'" />
                            <FormInput v-model="form2.last_name" :name="$t('Admin Last Name')"
                                :error="form2.errors.last_name" :type="'text'" :placeholder="'Last Name'"
                                :class="'w-1/2'" />
                        </div>
                        <FormInput v-model="form2.email" :name="$t('Admin Email')" :type="'email'"
                            :error="form2.errors.email" :placeholder="'Email'" :class="'sm:col-span-6 mb-4'" />
                        <FormInput v-model="form2.password" :name="$t('Admin Password')" :error="form2.errors.password"
                            :type="'password'" :placeholder="'Password'" :class="'sm:col-span-6 mb-4'" />
                        <div class="flex justify-between gap-x-2 mt-6">
                            <Link href="/install/database"
                                class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition-all duration-200 border border-slate-300 flex items-center space-x-2">
                            <ArrowLeft class="w-4 h-4" />
                ...
                <div v-else-if="props.step == 'migrations'" class="md:w-[30em] p-2">
                    ...
                    <div class="bg-slate-200 p-2 border-l-[3px] border-red-800 text-sm">
                        <p>{{ $t(`When you click proceed, the database installation process will begin. This might take
                            some time, so please hang tight and avoid closing the page`) }}.</p>
                    </div>
                        ...
                        <div class="flex justify-between gap-x-2">
                            ...
                            <button v-if="!isLoading" type="submit"
                                class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm">{{
                                    $t('Proceed') }}</button>
                            <button v-else type="button"
                                class="mt-4 rounded-md bg-primary px-3 py-2 text-sm text-white shadow-sm flex justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                                    <path fill="currentColor"
                                        d="M12 2A10 10 0 1 0 22 12A10 10 0 0 0 12 2Zm0 18a8 8 0 1 1 8-8A8 8 0 0 1 12 20Z"
                                        opacity=".5" />
