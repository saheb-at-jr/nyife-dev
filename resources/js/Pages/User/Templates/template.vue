  ...
  <AppLayout>
    <div class="h-[calc(100vh-65px)] bg-gradient-to-br from-slate-50 via-orange-50/30 to-slate-50 overflow-hidden">
        ...
        <div class="mx-auto px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
              <div class="w-12 h-12 bg-gradient-to-br from-[#ff5100] to-[#ff6422] rounded-xl flex items-center justify-center shadow-lg">
            ...
            <div class="flex items-center space-x-3">
              <Link href="/templates" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition-all duration-200 border border-slate-300 flex items-center space-x-2">
      ...
      <div class="flex h-[calc(100vh-152px)]">
          ...
          <div class="p-6 lg:p-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
              ...
              <div class="p-4 bg-slate-50 rounded-xl border-2 border-slate-200">
                <div class="flex items-center space-x-3">
                  <component :is="templateType === 'template' ? FileText : templateType === 'carousel' ? Layers : Workflow" 
                    class="w-6 h-6 text-[#ff5100]" />
                  <span class="text-sm font-medium text-slate-700">
                    {{ templateType === 'template' ? 'Template' : templateType === 'carousel' ? 'Carousel' : 'Flow' }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div v-if="templateType === 'template'" class="space-y-4">
              ...
              <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200 space-y-5">
                ...
                <FormInput v-model="form.name" :name="$t('Name')" :type="'text'" :disabled="true" :class="'w-full'" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormSelect v-model="form.category" :options="categoryOptions" :name="$t('Category')" :placeholder="$t('Select category')" />
                  <div>
                    <FormSelect v-model="form.language" :options="langOptions" :name="$t('Language')" :placeholder="$t('Select language')" :disabled="true" />
                    <span class="flex items-center gap-x-1 text-[11px] text-red-800 mt-1">
                      <AlertTriangle class="w-4 h-4" />
                      {{ $t('You are not allowed to edit this field') }}
                    </span>
              ...
              <div v-if="form.category === 'UTILITY' || form.category === 'MARKETING'"
                ...
                <!-- File Upload -->
                <div v-else class="border-2 border-dashed border-slate-300 rounded-xl p-8 hover:border-[#ff5100] transition-colors">
                  ...
                  <div v-if="form.header.example" class="flex items-center justify-center space-x-3 p-3 bg-slate-50 rounded-lg">
                    <component :is="form.header.format === 'IMAGE' ? ImageIcon : form.header.format === 'VIDEO' ? Video : FileIcon"
                      class="w-6 h-6 text-[#ff5100]" />
                    <span class="text-sm font-medium text-slate-700">{{ form.header.example.name || 'Existing file' }}</span>
                    <button @click="removeFile()" class="p-1 hover:bg-slate-200 rounded-full transition-colors">
                      <X class="w-4 h-4 text-slate-600" />
                    </button>
                  </div>

                  <label v-else :for="`file-upload-${form.header.format}`" class="cursor-pointer flex flex-col items-center space-y-3">
                    <component :is="form.header.format === 'IMAGE' ? ImageIcon : form.header.format === 'VIDEO' ? Video : FileIcon"
              ...
              <div v-if="form.category === 'UTILITY' || form.category === 'MARKETING'"
                ...
                <FormTextArea v-model="form.footer.text" :name="$t('Footer text')" :showLabel="false" :textAreaRows="2" />
                <span class="text-xs text-slate-500">{{ $t("Characters") }}: {{ footerCharacterCount }}/{{ footerCharacterLimit }}</span>
              </div>
                ...
                <div v-if="form.buttons.length > 0" class="space-y-3">
                  <div v-for="(button, index) in form.buttons" :key="index"
                    ...
                    <div class="flex items-center justify-between mb-3">
                      <span class="text-sm font-medium text-slate-700">{{ formatText(button.type) }}</span>
            ...
            <div v-if="templateType === 'carousel'" class="space-y-4">
              ...
              <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormSelect v-model="cardform.category" :options="categoryCrouselOptions" :name="$t('Category')" :placeholder="$t('Select category')" />
                  <div>
                    <FormSelect v-model="cardform.language" :options="langOptions" :name="$t('Language')" :placeholder="$t('Select language')" :disabled="true" />
                    <span class="flex items-center gap-x-1 text-[11px] text-red-800 mt-1">
                      <AlertTriangle class="w-4 h-4" />
                      {{ $t('You are not allowed to edit this field') }}
                    </span>
                  ...
                  <div class="mb-4">
                    ...
                    <div class="flex items-center space-x-3">
                      <label class="cursor-pointer flex items-center justify-center space-x-2 px-4 py-2 bg-gradient-to-r from-[#ff5100] to-[#ff6422] text-white rounded-lg hover:shadow-lg transition-all">
                        ...
                        <input type="file" accept="image/*" @change="(e) => onFileChange(e, cardIndex)" class="hidden" />
                      </label>
                      <span v-if="uploadingIndex === cardIndex" class="text-sm text-[#ff5100] flex items-center space-x-2">
                        <Loader2 class="w-4 h-4 animate-spin" />
                        <span>Uploading...</span>
                      </span>
                      <span v-else-if="cards[cardIndex].components[0].example.header_handle[0]" class="text-sm text-green-600 flex items-center space-x-2">
            ...
            <div v-if="templateType === 'flow'" class="space-y-4">
              ...
              <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200 space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormSelect v-model="flowform.category" :options="categoryCrouselOptions" :name="$t('Category')" :placeholder="$t('Select category')" />
                  <div>
                    <FormSelect v-model="flowform.language" :options="langOptions" :name="$t('Language')" :placeholder="$t('Select language')" :disabled="true" />
                    <span class="flex items-center gap-x-1 text-[11px] text-red-800 mt-1">
                      <AlertTriangle class="w-4 h-4" />
                      {{ $t('You are not allowed to edit this field') }}
                    </span>
              ...
              <div class="bg-white rounded-2xl shadow-sm p-6 border border-slate-200 space-y-4">
                ...
                <p class="text-sm text-slate-600 flex items-start space-x-2">
                  ...
                  <span>{{ $t("Note: Use the popup to modify your flow. Copy the JSON from there and paste it above.") }}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <!-- Right Panel - Preview -->
        <div class="w-full lg:w-1/2 bg-gradient-to-br from-slate-100 via-orange-50/20 to-slate-100 p-4 flex items-center justify-center relative">
          <!-- Background Decorations -->
          <div class="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-[#ff5100]/5 to-transparent rounded-full blur-3xl"></div>
          <div class="absolute bottom-0 left-0 w-80 h-80 bg-gradient-to-tr from-orange-200/20 to-transparent rounded-full blur-3xl"></div>

          <!-- Preview Container -->
          <div class="lg:sticky top-0 lg:z-10">
            <div class="text-center my-6">
              <div class="inline-flex items-center space-x-2 px-4 py-2 bg-white/80 backdrop-blur-sm rounded-full shadow-sm border border-slate-200 mb-3">
            ...
            <div class="bg-white rounded-3xl shadow-2xl p-0 border-8 border-slate-800 relative">
              <!-- Phone Notch -->
              <div class="absolute z-50 top-0 left-1/2 -translate-x-1/2 w-24 h-3 bg-slate-800 rounded-b-2xl"></div>
              ...
              <div class="w-full chat-bg pt-4 rounded-2xl overflow-hidden h-[calc(100vh-380px)] aspect-[12/16]">
                <div class="overflow-y-auto h-full p-4">
                  ...
                  <div v-if="templateType === 'template'" class="space-y-2">
                    <div class="mr-auto rounded-lg rounded-tl-none my-[0.1em] p-2 text-sm bg-white flex flex-col relative speech-bubble-left max-w-[60%] !text-wrap overflow-x-auto">
                      <!-- Header -->
                      <div v-if="form.header.format !== 'TEXT'"
    ...
    <Modal :label="''" :isOpen="isModalOpen" :showHeader="false">
      <div class="p-6">
        <div v-if="error" class="text-center">
          ...
          <h4 class="text-lg text-slate-800 font-semibold mb-2 text-start">{{ error.error?.error_user_title || 'Error' }}</h4>
...
// Import Lucide Icons
import {
  FileText, Info, ArrowLeft, Send, Loader2, MessageSquare, Zap, Layers, Workflow,
  Settings, ImageIcon, Video, FileIcon, AlignLeft, Phone, Link2, Copy, Trash2,
  Plus, Shield, Smartphone, Clock, MousePointerClick, Upload, CheckCircle,
...
onMounted(() => {
  if (props.template.name.startsWith('crousel_')) {
    templateType.value = 'carousel';
    initializeCarouselForm();
  } else if (props.template.name.startsWith('flow_')) {
...
function initializeCarouselForm() {
  ...
  if (carouselComponent && carouselComponent.cards) {
    cards.value = carouselComponent.cards.map(card => ({
